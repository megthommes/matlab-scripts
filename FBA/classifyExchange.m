...
produced1 = find(model1_flux > 0); % produced by model 1
produced2 = find(model2_flux > 0); % produced by model 2
consumed1 = find(model1_flux < 0); % consumed by model 1
consumed2 = find(model2_flux < 0); % consumed by model 2
notused1 = find(model1_flux == 0); % not used by model 1
notused2 = find(model2_flux == 0); % not used by model 2
...
fluxType(idx_con1_con2) = 2;
fluxType(idx_pro1_notused2) = 3;
fluxType(idx_notused1_pro2) = 4;
